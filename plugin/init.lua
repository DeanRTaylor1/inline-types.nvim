require("inline-types").setup()
-- local tsQuery = [[(call_expression
...
-- ]]
--
-- local method_names = vim.treesitter.query.parse("go", tsQuery)
--
...
-- end
--
-- local return_type_cache = {}
--
-- local getRetTypes = function(bufnr)
-- 	bufnr = bufnr or vim.api.nvim_get_current_buf()
-- 	--
-- 	-- Initialize cache for the buffer if it doesn't exist
-- 	return_type_cache[bufnr] = return_type_cache[bufnr] or {}
--
...
--
-- 	for id, node in method_names:iter_captures(root, bufnr, 0, -1) do
-- 		local name = method_names.captures[id]
-- 		if name == "method-name" or name == "function-name" then
-- 			local uri = vim.uri_from_bufnr(0)
-- 			local start_line, start_char = node:start()
--
-- 			if return_type_cache[bufnr][start_line] then
-- 				local virt_text = " " .. return_type_cache[bufnr][start_line]
...
-- end, {})
--
-- local augroup = vim.api.nvim_create_augroup("GoInlineTypes", { clear = true })
--
...
-- })
--
-- local extract_return_type = function(hover_response)
...
-- 	local signature = extract_return_type(result)
--
-- 	if not signature then
-- 		return
-- 	end
--
-- 	local rt = cleanSignature(signature)
--
-- 	callback(rt)
-- end
